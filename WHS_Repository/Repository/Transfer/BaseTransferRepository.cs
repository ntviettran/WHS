...
namespace WHS.Repository.Repository.Coordinate
{
    public class BaseTransferRepository : BaseRepository, ITransferRepository
    {
            ...
            try
            {
                ...
                var insertTransferSql = @"
                    insert into sys_npl_transfer 
                        (id, vehicle_id, plan_exec_date, plan_warehouse_date, exec_status, transfer_status, created_by, modified_by)
                    output inserted.id
                    values 
                        (@ID, @VehicleId, @PlanExecDate, @PlanWarehouseDate, @ExecStatus, @TransferStatus, @CreatedBy, @CreatedBy);
                ";
        ...
        public async Task<Response<int>> UpdateExecDate(int transferId)
        {
            ...
            try
            {
                string sql = $"update sys_npl_transfer set exec_date = getdate(), exec_status = {(int)E_TransferExec.DOING} where id = @transferId";
