...
namespace WHS.Pages.Receive
{
    public partial class GroupDetailReceive : UserControl
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            detailReceivePage.SetType(_type);
        ...
        private async Task LoadDataGridView()
        {
            ...
            switch (_type)
            {
                case E_NPLType.FABRIC:
                    IReceiveService<FabricDto, FabricReceivedDto> _fabricService = ServiceFactory.GetReceiveService<FabricDto, FabricReceivedDto>();
                    res = await _fabricService.GetGroupedReceiveAsync(paginate, receiveSearch);
                    break;
                case E_NPLType.PLSP:
                    IReceiveService<PlspDto, PlspReceivedDto> _plspService = ServiceFactory.GetReceiveService<PlspDto, PlspReceivedDto>();
                    res = await _plspService.GetGroupedReceiveAsync(paginate, receiveSearch);
                    break;
                case E_NPLType.PLDG:
                    IReceiveService<PldgDto, PldgReceivedDto> _pldgService = ServiceFactory.GetReceiveService<PldgDto, PldgReceivedDto>();
